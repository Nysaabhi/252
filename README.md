...
function showLocatorsOverlay() {
  ...
  if (exactMatches.length > 0) {
    ...
    updateLocatorActiveFilterButton(filterButtons, null, null, null);
    return;
  }

  // Advanced filtering with scoring system
  const filteredLocators = filterAndScoreLocators(locators, normalizedQuery, categorySynonyms, citySynonyms);
  ...
  // Update active filter button based on search
  updateLocatorActiveFilterButton(filterButtons, 
    filteredLocators[0]?.categoryMatch, 
    filteredLocators[0]?.cityMatch, 
    filteredLocators[0]?.productMatch);
}

// Helper function to find exact matches (name + address combinations)
function findExactMatches(locators, normalizedQuery) {
  const exactMatches = [];
  
  // Check for patterns like "dmart in jahangirabad" or "is there any dmart in neelam park"
  const locationPattern = /(?:in|at|near|around)\s+([a-z\s]+)/i;
  const locationMatch = normalizedQuery.match(locationPattern);
  
  // Extract store name (everything before location indicator)
  let storeName = '';
  if (locationMatch) {
    storeName = normalizedQuery.split(/(?:in|at|near|around)/i)[0].trim();
  }
  ...
  locators.forEach(locator => {
    ...
    // Case 2: Name + address combination match
    if (storeName && locationMatch) {
      const location = locationMatch[1].trim();
      
      // Check if name matches and address contains the location
    ...
    // Case 3: Exact address match
    if (locatorAddress.includes(normalizedQuery)) {
      exactMatches.push(locator);
      return;
    }
    
    // Case 4: Pincode match
    const pincodeMatch = normalizedQuery.match(/\b\d{6}\b/);
    if (pincodeMatch && locatorAddress.includes(pincodeMatch[0])) {
      exactMatches.push(locator);
      return;
    }
  });
...
// Main filtering and scoring function
function filterAndScoreLocators(locators, normalizedQuery, categorySynonyms, citySynonyms) {
  // Parse query for different components
  const { categoryMatch, cityMatch, priceMatch, productMatch, 
          qualityMatch, ratingMatch, brandMatch, hasNegation } = parseQueryComponents(normalizedQuery, categorySynonyms, citySynonyms);
  ...
  return locators.map(locator => {
    ...
    if (categoryMatch) {
      ...
      if (categoryLower.includes(categoryMatch.toLowerCase())) {
        score += 30;
    ...
    if (cityMatch) {
      ...
      if (cityLower.includes(cityMatch.toLowerCase())) {
        score += 30;
    ...
    // Name matching (very high importance)
    const nameMatchScore = calculateNameMatchScore(locator.details.name, normalizedQuery);
    if (nameMatchScore > 0) {
      score += nameMatchScore * 40;
      matches.push(`Name match (${nameMatchScore.toFixed(2)})`);
    } else if (brandMatch || categoryMatch || cityMatch) {
      // If we have specific filters but name doesn't match at all, exclude
      return null;
    }

    // Address matching (high importance for location-specific queries)
    const addressMatchScore = calculateAddressMatchScore(locator.details.address, normalizedQuery);
    if (addressMatchScore > 0) {
      score += addressMatchScore * 35; // Higher weight for address matches
      matches.push(`Address match (${addressMatchScore.toFixed(2)})`);
    }

    // Other matching criteria (price, products, etc.)
    score += evaluateOtherCriteria(locator, priceMatch, productMatch, qualityMatch, ratingMatch, brandMatch, matches, mismatches);

    // Handle negation queries
    if (hasNegation && checkNegation(locator, normalizedQuery)) {
      return null;
    }

    // If no specific matches but general text search
    if (score === 0) {
      const generalMatchScore = evaluateGeneralSearch(locator, normalizedQuery);
...
// Parse query into different components
function parseQueryComponents(normalizedQuery, categorySynonyms, citySynonyms) {
  let categoryMatch, cityMatch, priceMatch, productMatch, qualityMatch, ratingMatch, brandMatch;
  let hasNegation = false;

  // Check for negation terms
  hasNegation = /\b(no|not|without|except)\b/.test(normalizedQuery);

  // 1. Category detection with synonyms
  const categoryPattern = new RegExp(
    `\\b(${Object.keys(categorySynonyms).join('|')}|grocery|food|electronics|fashion|pharmacy)\\b`, 'i');
  categoryMatch = normalizedQuery.match(categoryPattern);
  if (categoryMatch) {
    const matchedTerm = categoryMatch[0].toLowerCase();
    categoryMatch = categorySynonyms[matchedTerm] || matchedTerm.charAt(0).toUpperCase() + matchedTerm.slice(1);
  }

  // 2. City detection with synonyms
  const cityPattern = new RegExp(
    `\\b(${Object.keys(citySynonyms).join('|')}|delhi|mumbai|bhopal|bangalore|kolkata|chennai|hyderabad|pune)\\b`, 'i');
  cityMatch = normalizedQuery.match(cityPattern);
  if (cityMatch) {
    const matchedTerm = cityMatch[0].toLowerCase();
    cityMatch = citySynonyms[matchedTerm] || matchedTerm.charAt(0).toUpperCase() + matchedTerm.slice(1);
  }
  ...
  // 4. Product detection
  const productTerms = ['oil', 'salt', 'rice', 'atta', 'sugar', 'burger', 'pizza', 'phone', 'tv', 'shirt', 'medicine'];
  productMatch = normalizedQuery.match(new RegExp(`\\b(${productTerms.join('|')})\\b`, 'i'));

  // 5. Quality detection
  qualityMatch = normalizedQuery.match(/(cheap|affordable|budget|expensive|premium)/i);

  // 6. Rating detection
  ratingMatch = normalizedQuery.match(/(\d+(?:\.\d+)?)\s*(?:star|rating)/i);

  // 7. Brand detection
  brandMatch = normalizedQuery.match(/(dmart|burger king|reliance digital|big bazaar|dominos)/i);

  return { categoryMatch, cityMatch, priceMatch, productMatch, qualityMatch, ratingMatch, brandMatch, hasNegation };
}

// Calculate name match score with priority to exact matches
function calculateNameMatchScore(name, query) {
...
// Calculate address match score with priority to exact location mentions
function calculateAddressMatchScore(address, query) {
  const addressLower = address.toLowerCase();
  
  // Exact address match
  if (addressLower.includes(query)) return 1.0;
  
  // Check for location patterns like "in jahangirabad"
  const locationPattern = /(?:in|at|near|around)\s+([a-z\s]+)/i;
  ...
  let matchedWords = 0;
  queryWords.forEach(qWord => {
    if (qWord.length >= 3 && addressLower.includes(qWord)) matchedWords++;
  });
  
  return queryWords.length > 0 ? matchedWords / queryWords.length : 0;
}

// Evaluate other matching criteria
function evaluateOtherCriteria(locator, priceMatch, productMatch, qualityMatch, ratingMatch, brandMatch, matches, mismatches) {
  let score = 0;

  // Price matching
  if (priceMatch) {
    const price = parseInt(priceMatch[1]);
    const hasAffordableProducts = locator.details.services.some(service => {
      const servicePrice = parseInt(service.price.replace(/[^\d]/g, ''));
      return servicePrice <= price;
    });
    
    if (hasAffordableProducts) {
      score += 20;
      matches.push(`Price: ≤₹${price}`);
    } else {
      mismatches.push(`Price: ≤₹${price}`);
  ...
  // Product matching
  if (productMatch) {
    const hasMatchingProducts = locator.details.services.some(service => 
      service.name.toLowerCase().includes(productMatch[0]) || 
...
// Check for negation terms
function checkNegation(locator, query) {
  const negatedTerms = query.match(/\b(no|not|without|except)\s+(\w+)/i);
  if (negatedTerms) {
    const negatedTerm = negatedTerms[2];
    const searchFields = [
      locator.category,
    ...
    return searchFields.includes(negatedTerm);
  }
  return false;
}

// Evaluate general search when no specific components matched
function evaluateGeneralSearch(locator, query) {
  const searchFields = [
    locator.category,
    locator.metropolis,
    locator.details.name,
...
function showNoResults(gridElement, query, locators) {
  const closeMatches = findCloseMatches(locators, query.toLowerCase());
      ...
      <div class="no-results">
        ...
        <div class="close-matches">
          ${closeMatches.slice(0, 3).map(match => `
            <div class="close-match" onclick="document.getElementById('locatorsExploreInput').value='${match.name}'; performSearch(locatorsData, '${match.name}', document.getElementById('locatorsGrid'), document.querySelectorAll('.filter-button'), renderLocatorCards)">
              ${match.name}
            </div>
          `).join('')}
        </div>
...
function findCloseMatches(locators, query) {
  ...
  locators.forEach(locator => {
    const name = locator.details.name.toLowerCase();
    const distance = levenshteinDistance(query, name.substring(0, query.length));
    
    if (distance <= MAX_DISTANCE) {
      matches.push({
        name: locator.details.name,
        distance
      });
    }
  });
...
function updateLocatorActiveFilterButton(filterButtons, categoryMatch, cityMatch, productMatch) {
  filterButtons.forEach(btn => {
    ...
    if (categoryMatch && btn.dataset.category === categoryMatch) {
      btn.classList.add('active');
    } else if (cityMatch && btn.dataset.city === cityMatch) {
      btn.classList.add('active');
    } else if (productMatch && btn.dataset.product === productMatch[0]) {
      btn.classList.add('active');
    } else if (!categoryMatch && !cityMatch && !productMatch && btn.dataset.category === 'All') {
      btn.classList.add('active');
    }
  });
}
